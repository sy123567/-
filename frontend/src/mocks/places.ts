import type { NodeType, PlaceDetail } from "../types";

export const placeDetails: Record<string, PlaceDetail> = {
  上海外滩: {
    placeName: "上海外滩",
    code: "SHA",
    category: "城市地标",
    intro: "沿黄浦江展开的城市天际线入口，一侧是江风与轮渡，一侧是成排的历史建筑。",
    highlights: ["外滩源与万国建筑群", "黄浦江观景步道", "浦东陆家嘴天际线", "外白渡桥"],
    foods: [{ name: "生煎馒头", desc: "皮脆汤足，适合上午边走边吃。" }, { name: "本帮菜", desc: "红烧肉、油爆虾是上海味道的经典组合。" }],
    bestTime: "日出后或日落前一小时",
    ticket: "沿江步道免费",
    tips: "周末晚间人流较大，建议从外滩源向南慢慢走。",
  },
  老吉士: {
    placeName: "老吉士",
    code: "LJS",
    category: "本帮餐厅",
    intro: "以家常本帮菜见长的老牌餐厅，适合把午餐安排成一段正式但不拘谨的城市体验。",
    highlights: ["招牌红烧肉", "蟹粉豆腐", "油爆虾", "浓油赤酱的本帮风味"],
    foods: [{ name: "红烧肉", desc: "肥瘦相间，酱汁浓郁，建议搭配米饭。" }, { name: "蟹粉豆腐", desc: "口感细腻，适合多人分食。" }, { name: "油爆虾", desc: "甜咸鲜明，是桌上很稳妥的选择。" }],
    bestTime: "11:30 前到店可避开午餐高峰",
    ticket: "人均约 ¥120—180",
    tips: "多人用餐建议提前确认排队和座位情况。",
  },
  豫园: {
    placeName: "豫园",
    code: "YUY",
    category: "园林与老城",
    intro: "豫园、城隍庙与老城厢相连，园林、街巷和小吃集中在一片可步行探索的区域。",
    highlights: ["豫园古典园林", "九曲桥与湖心亭", "城隍庙老街", "福佑路小商品街"],
    foods: [{ name: "南翔小笼", desc: "皮薄汁多，适合错峰排队。" }, { name: "桂花酒酿圆子", desc: "甜香温润，适合作为下午点心。" }, { name: "蟹壳黄", desc: "酥皮咸香，适合边走边吃。" }],
    bestTime: "工作日上午或 16:00 后",
    ticket: "园林门票约 ¥40",
    tips: "雨天路面湿滑，九曲桥附近建议放慢脚步。",
  },
  上海外滩茂悦大酒店: {
    placeName: "上海外滩茂悦大酒店",
    code: "HYT",
    category: "住宿",
    intro: "位于北外滩一带的城市酒店，适合作为外滩与浦东景观路线的夜间落脚点。",
    highlights: ["北外滩江景", "便捷连接外滩步道", "适合团队集合", "城市夜景视野"],
    foods: [{ name: "早餐自助", desc: "适合早起出发前快速补充能量。" }, { name: "江景下午茶", desc: "适合把行程留出一段缓冲时间。" }],
    bestTime: "办理入住通常为 14:00 后",
    ticket: "房价随日期浮动",
    tips: "多人入住请提前确认房型和早餐人数。",
  },
  上海博物馆: {
    placeName: "上海博物馆",
    code: "SHM",
    category: "博物馆",
    intro: "以中国古代艺术收藏见长的城市博物馆，适合在天气变化时作为室内替代节点。",
    highlights: ["青铜器馆", "陶瓷馆", "书画馆", "人民广场城市中轴线"],
    foods: [{ name: "博物馆周边本帮面", desc: "适合参观前后快速用餐。" }, { name: "人民广场咖啡", desc: "在展览之间留出一段休息时间。" }],
    bestTime: "开馆后第一小时或工作日下午",
    ticket: "以预约页面为准",
    tips: "热门展览可能需要提前预约，留意入馆时间。",
  },
  宽窄巷子: {
    placeName: "宽窄巷子",
    code: "KZX",
    category: "历史街区",
    intro: "由宽巷子、窄巷子和井巷子组成的成都老城街区，适合慢走、喝茶和看院落。",
    highlights: ["老成都院落", "井巷子砖墙", "茶馆与盖碗茶", "夜间街巷灯影"],
    foods: [{ name: "担担面", desc: "麻辣鲜香，建议按口味选择辣度。" }, { name: "钟水饺", desc: "红油香气明显，适合小份尝鲜。" }],
    bestTime: "上午 9:00—11:00 或傍晚",
    ticket: "街区免费",
    tips: "节假日人流密集，建议从井巷子一侧进入。",
  },
  成都大熊猫繁育研究基地: {
    placeName: "成都大熊猫繁育研究基地",
    code: "CDP",
    category: "自然与动物",
    intro: "成都最具代表性的熊猫主题目的地，清晨是观察熊猫活动状态的好时段。",
    highlights: ["熊猫别墅", "幼年熊猫活动区", "大熊猫博物馆", "竹林步道"],
    foods: [{ name: "成都甜水面", desc: "甜辣酱香，适合午餐前后尝鲜。" }, { name: "盖碗茶", desc: "回到市区后安排一段慢节奏休息。" }],
    bestTime: "开园后至 11:00",
    ticket: "门票与预约以官方公告为准",
    tips: "园区较大，穿舒适鞋并预留半天时间。",
  },
  西湖: {
    placeName: "西湖",
    code: "WES",
    category: "湖山风景",
    intro: "杭州城市气质的核心，一段环湖路线可以把桥、堤、山色和旧街区串在一起。",
    highlights: ["断桥残雪", "苏堤春晓", "曲院风荷", "雷峰塔远眺"],
    foods: [{ name: "西湖醋鱼", desc: "酸甜口味明显，适合多人分食。" }, { name: "龙井虾仁", desc: "清鲜细嫩，是杭州菜的代表。" }],
    bestTime: "清晨或日落前",
    ticket: "环湖步道免费，部分景点另计",
    tips: "骑行和步行路线较长，建议按体力选择一段。",
  },
  河坊街: {
    placeName: "河坊街",
    code: "HFJ",
    category: "老街与美食",
    intro: "南宋御街附近的传统商业街，适合把地方小吃、手作店和夜间散步放在一起。",
    highlights: ["胡庆余堂", "南宋御街连接线", "传统手作店", "夜间小吃摊"],
    foods: [{ name: "定胜糕", desc: "软糯清甜，适合作为街头点心。" }, { name: "葱包桧", desc: "酥脆咸香，现做口感更好。" }],
    bestTime: "傍晚至夜间",
    ticket: "街区免费",
    tips: "热门时段注意保管随身物品，店铺营业时间各不相同。",
  },
  秦始皇帝陵博物院: {
    placeName: "秦始皇帝陵博物院",
    code: "QIN",
    category: "世界遗产",
    intro: "以兵马俑坑闻名的大型遗址博物馆，建议按一号坑、三号坑、二号坑的顺序参观，预留半天时间。",
    highlights: ["一号坑兵马俑军阵", "铜车马展厅", "二号坑与三号坑", "秦始皇陵封土远眺"],
    foods: [{ name: "临潼火晶柿子", desc: "季节限定，甜软多汁，出园后可尝。" }, { name: "陕西臊子面", desc: "酸辣开胃，适合参观后补充体力。" }],
    bestTime: "开馆后第一小时，避开团队高峰",
    ticket: "门票以官方预约页面为准，建议提前实名预约",
    tips: "园区面积大且步行多，穿舒适鞋并预留 3–4 小时，展厅内注意跟紧讲解节奏。",
  },
  大雁塔: {
    placeName: "大雁塔",
    code: "DYT",
    category: "历史地标",
    intro: "大慈恩寺内的唐代砖塔，周边的大雁塔北广场与大唐不夜城适合串成一段傍晚到夜间的路线。",
    highlights: ["大慈恩寺", "登塔俯瞰城市", "北广场音乐喷泉", "大唐不夜城步行街"],
    foods: [{ name: "肉夹馍", desc: "腊汁肉配白吉馍，街头即可购得。" }, { name: "冰峰汽水", desc: "本地经典搭配，解腻消暑。" }],
    bestTime: "傍晚观塔，夜间看喷泉与灯光",
    ticket: "寺院与登塔分开计费，以现场公告为准",
    tips: "喷泉表演有固定场次，出发前确认当天时间。",
  },
  西安城墙: {
    placeName: "西安城墙",
    code: "XAW",
    category: "历史地标",
    intro: "保存完整的明代城墙，可步行或骑行环城一周，适合在早晚人流较少时安排。",
    highlights: ["永宁门（南门）", "城墙骑行环线", "角楼与箭楼", "护城河与环城公园"],
    foods: [{ name: "biángbiáng 面", desc: "宽面劲道，油泼辣子香气足。" }, { name: "甑糕", desc: "枣香软糯，适合作为早点。" }],
    bestTime: "清晨或日落前，光线与温度更适宜",
    ticket: "登城门票以官方渠道为准，自行车可另租",
    tips: "骑行环线约 13.7 公里，量力而行并留意还车时间。",
  },
  回民街: {
    placeName: "回民街",
    code: "HMJ",
    category: "老街与美食",
    intro: "钟鼓楼后方的传统美食街区，由多条巷子组成，适合把晚餐和夜间散步安排在一起。",
    highlights: ["北院门老街", "化觉巷清真大寺", "各色小吃摊", "夜间街巷灯影"],
    foods: [{ name: "羊肉泡馍", desc: "自己掰馍再煮，汤浓馍筋。" }, { name: "肉夹馍与镜糕", desc: "边走边吃的经典街头组合。" }],
    bestTime: "傍晚至夜间",
    ticket: "街区免费，小吃单点",
    tips: "人流密集，注意保管随身物品并错峰前往热门摊位。",
  },
};

const categoryLabels: Record<NodeType, string> = {
  ATTRACTION: "景点",
  MEAL: "餐饮",
  LODGING: "住宿",
  TRANSPORT: "交通",
  OTHER: "行程节点",
};

export interface SuggestedPlace {
  city: string;
  placeName: string;
  latitude: number;
  longitude: number;
  nodeType: NodeType;
  durationMinutes: number;
}

const suggestedPlaces: SuggestedPlace[] = [
  { city: "上海", placeName: "上海外滩", latitude: 31.2304, longitude: 121.4737, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "上海", placeName: "豫园", latitude: 31.2272, longitude: 121.492, nodeType: "ATTRACTION", durationMinutes: 150 },
  { city: "上海", placeName: "老吉士", latitude: 31.2113, longitude: 121.4512, nodeType: "MEAL", durationMinutes: 90 },
  { city: "北京", placeName: "故宫博物院", latitude: 39.9163, longitude: 116.3972, nodeType: "ATTRACTION", durationMinutes: 210 },
  { city: "北京", placeName: "颐和园", latitude: 39.9999, longitude: 116.2755, nodeType: "ATTRACTION", durationMinutes: 180 },
  { city: "北京", placeName: "南锣鼓巷", latitude: 39.937, longitude: 116.4039, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "成都", placeName: "成都大熊猫繁育研究基地", latitude: 30.7338, longitude: 104.144, nodeType: "ATTRACTION", durationMinutes: 210 },
  { city: "成都", placeName: "宽窄巷子", latitude: 30.6573, longitude: 104.0665, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "成都", placeName: "武侯祠", latitude: 30.6496, longitude: 104.0482, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "杭州", placeName: "西湖", latitude: 30.244, longitude: 120.149, nodeType: "ATTRACTION", durationMinutes: 180 },
  { city: "杭州", placeName: "河坊街", latitude: 30.2431, longitude: 120.1715, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "杭州", placeName: "灵隐寺", latitude: 30.2401, longitude: 120.1015, nodeType: "ATTRACTION", durationMinutes: 150 },
  { city: "西安", placeName: "秦始皇帝陵博物院", latitude: 34.3848, longitude: 109.2731, nodeType: "ATTRACTION", durationMinutes: 210 },
  { city: "西安", placeName: "西安城墙", latitude: 34.2583, longitude: 108.947, nodeType: "ATTRACTION", durationMinutes: 150 },
  { city: "西安", placeName: "回民街", latitude: 34.2644, longitude: 108.9487, nodeType: "MEAL", durationMinutes: 120 },
  { city: "广州", placeName: "广州塔", latitude: 23.1066, longitude: 113.3246, nodeType: "ATTRACTION", durationMinutes: 150 },
  { city: "广州", placeName: "陈家祠", latitude: 23.1258, longitude: 113.2529, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "广州", placeName: "沙面岛", latitude: 23.109, longitude: 113.2385, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "重庆", placeName: "洪崖洞", latitude: 29.563, longitude: 106.5805, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "重庆", placeName: "长江索道", latitude: 29.558, longitude: 106.585, nodeType: "ATTRACTION", durationMinutes: 90 },
  { city: "重庆", placeName: "磁器口古镇", latitude: 29.5796, longitude: 106.4477, nodeType: "ATTRACTION", durationMinutes: 150 },
  { city: "南京", placeName: "中山陵", latitude: 32.0584, longitude: 118.8486, nodeType: "ATTRACTION", durationMinutes: 180 },
  { city: "南京", placeName: "夫子庙", latitude: 32.0227, longitude: 118.787, nodeType: "ATTRACTION", durationMinutes: 120 },
  { city: "南京", placeName: "南京博物院", latitude: 32.0405, longitude: 118.8343, nodeType: "ATTRACTION", durationMinutes: 180 },
  { city: "大理", placeName: "大理古城", latitude: 25.696, longitude: 100.161, nodeType: "ATTRACTION", durationMinutes: 150 },
  { city: "大理", placeName: "洱海", latitude: 25.786, longitude: 100.19, nodeType: "ATTRACTION", durationMinutes: 210 },
  { city: "大理", placeName: "苍山", latitude: 25.679, longitude: 100.12, nodeType: "ATTRACTION", durationMinutes: 180 },
];

export function suggestPlaces(query: string, city = ""): SuggestedPlace[] {
  const keyword = query.trim().toLowerCase();
  const scope = city.trim().toLowerCase();
  // 限定在目的地城市内，避免选到同名的异地地点。
  const pool = scope ? suggestedPlaces.filter((place) => place.city.toLowerCase().includes(scope)) : suggestedPlaces;
  if (!keyword) return pool.slice(0, 8);
  return pool.filter((place) => `${place.city}${place.placeName}`.toLowerCase().includes(keyword)).slice(0, 8);
}

export function getCitySuggestions(city: string): SuggestedPlace[] {
  const keyword = city.trim().toLowerCase();
  if (!keyword) return [];
  return suggestedPlaces.filter((place) => place.city.toLowerCase().includes(keyword));
}

const placeImages: Record<string, string> = {
  上海外滩: "https://images.unsplash.com/photo-1548919973-5cef591cdbc9?auto=format&fit=crop&w=900&q=80",
  老吉士: "https://images.unsplash.com/photo-1515003197210-e0cd71810b5f?auto=format&fit=crop&w=900&q=80",
  豫园: "https://images.unsplash.com/photo-1523731407965-2430cd12f5e4?auto=format&fit=crop&w=900&q=80",
  上海外滩茂悦大酒店: "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=900&q=80",
  上海博物馆: "https://images.unsplash.com/photo-1564399579883-451a5d44ec08?auto=format&fit=crop&w=900&q=80",
  宽窄巷子: "https://images.unsplash.com/photo-1548013146-72479768bada?auto=format&fit=crop&w=900&q=80",
  成都大熊猫繁育研究基地: "https://images.unsplash.com/photo-1564349683136-77e08dba1ef7?auto=format&fit=crop&w=900&q=80",
  西湖: "https://images.unsplash.com/photo-1538485399081-7c897a9a3d20?auto=format&fit=crop&w=900&q=80",
  河坊街: "https://images.unsplash.com/photo-1474181487882-5abf3f0ba6c2?auto=format&fit=crop&w=900&q=80",
};

const nodeTypeImages: Record<NodeType, string> = {
  ATTRACTION: "https://images.unsplash.com/photo-1500534623283-312aade485b7?auto=format&fit=crop&w=900&q=80",
  MEAL: "https://images.unsplash.com/photo-1515003197210-e0cd71810b5f?auto=format&fit=crop&w=900&q=80",
  LODGING: "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=900&q=80",
  TRANSPORT: "https://images.unsplash.com/photo-1514565131-fce0801e5785?auto=format&fit=crop&w=900&q=80",
  OTHER: "https://images.unsplash.com/photo-1470770841072-f978cf4d019e?auto=format&fit=crop&w=900&q=80",
};

export function getPlaceImage(placeName: string, nodeType: NodeType = "OTHER"): string {
  return placeImages[placeName] ?? nodeTypeImages[nodeType];
}

export function getPlaceDetail(placeName: string, nodeType: NodeType = "OTHER"): PlaceDetail {
  return placeDetails[placeName] ?? {
    placeName,
    code: undefined,
    category: categoryLabels[nodeType],
    intro: `${placeName} 是这段路线中的一个重要停靠点，平台会把它的时间、费用和团队约束一起纳入行程判断。`,
    highlights: ["路线中的重点停靠", "适合按现场状态灵活调整", "可与相邻节点组合探索"],
    foods: [{ name: "当地特色", desc: "可根据现场评价和成员饮食需求选择。" }, { name: "附近简餐", desc: "为下一段路线预留足够的用餐时间。" }],
    bestTime: "以当天行程和现场人流为准",
    ticket: "费用以现场或官方渠道为准",
    tips: "出发前确认开放时间、预约要求和天气变化。",
  };
}
