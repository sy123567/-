...
三人团队按照用户与社区（王一飞）、组队与出行规划（祖建明）、监测应变与后台及智能能力（宋研）进行分工，可以复用 Spring Boot、React 和 MyBatis 的成熟生态，减少重复开发。对于地图服务、天气服务等可能产生费用的外部资源，系统提供估算路线和 Mock 事件作为降级方案，因此课程设计阶段不依赖付费服务即可完成主要功能验证。
...
系统按照用户可理解的业务边界划分为七大分组：

1. **账号管理**：负责注册和登录、注销、个人资料管理，并提供 JWT 鉴权和角色权限能力。
2. **社交组队管理**：负责好友关系、出行小组和房间码加入，为后续成员约束与共同决策提供参与者集合。
3. **出行规划管理**：负责成员约束、初始方案、行程节点、地图路线和预算费用，是生成可执行行程的基础；AI 辅助行程生成作为初始方案生成的一种输入方式并入本分组，其产物与手工节点走同一条路线与预算链路。
4. **监测与应变管理**：负责外部事件接入、影响匹配、风险评分、持续动态分析、自动重规划、投票、变更记录和消息通知，是系统核心难点闭环。
5. **社区管理**：负责讨论、攻略浏览、攻略纳用和行后游记，形成从内容消费到旅行反馈的闭环。
6. **智能能力管理**：负责全局旅行助手、攻略与功能目录检索、替代地点智能提名以及模型不可用时的降级策略；模型调用统一在服务端完成，密钥不下发浏览器。
7. **后台与监控管理**：负责用户管理、运营后台和数据统计。
...
用例图：[账号管理用例图](uml/用例图_账号管理.png)。时序图：[账号管理时序图](uml/时序图_账号管理.png)；活动图：[账号管理活动图](uml/活动图_账号管理.png)。以下表格均采用学校标准九字段用例描述格式。
...
| 涉及的文档和数据 | 实体 User；表 `users(id, name, email, phone, password_hash, role, status, created_at)`；接口 DTO `RegisterRequest(name, email, phone, password, avatar)`、`UserVO`；响应字段 `user.id/name/email/role/status`。 |
...
| 涉及的文档和数据 | 实体 User、AuthSession；表 `users(id, email, password_hash, role, status)`；Redis `auth:token:{jti}`；接口 DTO `LoginRequest(email, password)`、`AuthResponse(user, token, expiresIn)`、JWT claims `sub/jti/exp`。 |
...
| 涉及的文档和数据 | 实体 User、AuthSession；表 `users(id, status, updated_at)`；Redis `auth:token:{jti}`；接口对象 `LogoutRequest`、`Result<Void>`，关键字段为 `jti`、`expiresAt`。 |
...
| 涉及的文档和数据 | 实体 User；表 `users(id, name, email, phone, avatar, updated_at, deleted_at)`；接口 DTO `UpdateProfileRequest(name, email, phone, avatar)`、`UserVO`。 |
| 后置条件 | 用户资料和头像地址被持久化。 |
...
功能分组用例图：[社交组队管理用例图](uml/用例图_社交组队管理.png)。时序图：[社交组队管理时序图](uml/时序图_社交组队管理.png)；活动图：[社交组队管理活动图](uml/活动图_社交组队管理.png)。
...
| 涉及的文档和数据 | 实体 User、Friendship；表 `users(id, name, email, phone, avatar)`、`friendships(id, requester_id, addressee_id, pair_key, status, created_at)`；接口 DTO `FriendRequest(addresseeId)`、`FriendshipView`。 |
...
| 涉及的文档和数据 | 实体 User、TravelGroup、GroupMember；表 `travel_groups(id, name, description, owner_user_id, room_code, room_code_expire_at, member_limit, status)`、`group_members(id, group_id, user_id, role, status, joined_at, left_at)`；接口 DTO `CreateGroupRequest(name, description, memberLimit)`、`TravelGroupVO`、`GroupMemberVO`。 |
...
| 涉及的文档和数据 | 实体 TravelGroup、GroupMember、MemberConstraint；表 `travel_groups(id, room_code, room_code_expire_at, member_limit, status)`、`group_members(group_id, user_id, role, status, joined_at)`、`member_constraints(member_id, available_from, available_to, max_budget)`；Redis `room:{roomCode}`；接口 DTO `JoinGroupRequest(roomCode)`、`JoinGroupResult(group, status)`。 |
...
功能分组用例图：[出行规划管理用例图](uml/用例图_出行规划管理.png)。时序图：[出行规划管理时序图](uml/时序图_出行规划管理.png)；活动图：[出行规划管理活动图](uml/活动图_出行规划管理.png)。
...
| 涉及的文档和数据 | 实体 GroupMember、MemberConstraint；表 `group_members(id, group_id, user_id, status)`、`member_constraints(id, member_id, available_from, available_to, max_budget, must_visit_places, fitness_level, dietary_needs, accessibility_needs, updated_at)`；接口 DTO `ConstraintRequest`、`MemberConstraintVO`。 |
...
| 涉及的文档和数据 | 实体 MemberConstraint、Trip、ItineraryNode、Route；表 `member_constraints(member_id, available_from, available_to, max_budget, must_visit_places)`、`trips(id, group_id, title, status, start_date, end_date, total_budget, spent_budget, version)`、`itinerary_nodes(trip_id, name, place_name, node_type, planned_start, planned_end, cost, sequence_order, status)`、`routes(trip_id, from_node_id, to_node_id, transport_mode, distance_km, duration_minutes, cost)`；接口 DTO `GeneratePlanRequest(startDate, title)`、`PlanVO(trip, tradeOffExplanation)`。 |
...
| 涉及的文档和数据 | 实体 Trip、ItineraryNode、Route；表 `trips(id, group_id, version, status)`、`itinerary_nodes(id, trip_id, name, place_name, latitude, longitude, node_type, planned_start, planned_end, cost, sequence_order, status, updated_at)`、`routes(trip_id, from_node_id, to_node_id)`；接口 DTO `NodeCreateRequest`、`NodeUpdateRequest`、`ReorderNodesRequest(nodeIds)`、`ItineraryNodeVO`。 |
| 后置条件 | 节点内容、顺序和状态已持久化。 |
...
| 涉及的文档和数据 | 实体 Trip、ItineraryNode、Route；表 `itinerary_nodes(id, trip_id, latitude, longitude, sequence_order)`、`routes(id, trip_id, from_node_id, to_node_id, transport_mode, distance_km, duration_minutes, cost, polyline, source)`；接口 DTO `RoutePlanRequest(tripId, fromNodeId, toNodeId, transportMode)`、`RouteVO`。 |
...
| 涉及的文档和数据 | 实体 Trip、Bill、BillShare；表 `trips(id, total_budget, spent_budget)`、`bills(id, trip_id, title, amount, category, paid_by_member_id, split_mode, occurred_at)`、`bill_shares(bill_id, member_id, share_amount, share_ratio)`；接口 DTO `ExpenseRequest(title, amount, category, paidByMemberId, nodeId, occurredAt)`、`BudgetVO(totalBudget, spentBudget, remainingBudget, overBudget, expenses)`、`BillVO`。 |
...
功能分组用例图：[监测与应变管理用例图](uml/用例图_监测与应变管理.png)。完整时序图：[监测与应变管理时序图](uml/时序图_监测与应变管理.png)；活动图：[监测与应变管理活动图](uml/活动图_监测与应变管理.png)。投票详图：[投票决策活动图](uml/活动图_投票决策.png)。
...
| 涉及的文档和数据 | 实体 ExternalEvent；表 `external_events(id, event_type, title, description, place_name, latitude, longitude, radius_km, severity, start_time, end_time, source, status, payload)`；接口 DTO `EventCreateRequest`、`ExternalEventVO`，接口 `POST /api/events`、`GET /api/events/active`。 |
...
| 涉及的文档和数据 | 实体 ExternalEvent、Trip、ItineraryNode、ImpactAssessment；表 `external_events(id, latitude, longitude, radius_km, start_time, end_time, severity, status)`、`itinerary_nodes(id, trip_id, latitude, longitude, planned_start, planned_end, status)`、`impact_assessments(trip_id, event_id, affected_node_id, risk_score, impact_level, assessment_hash, assessed_at)`；接口 DTO `AssessResponse`、`ImpactAssessmentVO`。 |
| 后置条件 | 影响评估和受影响节点被持久化。 |
...
| 涉及的文档和数据 | 实体 ImpactAssessment、Trip；表 `impact_assessments(trip_id, affected_node_id, risk_score, impact_level, description, assessed_at)`、`trips(id, status, start_date, end_date)`；接口 DTO `RiskSnapshotVO(score, riskLabel, factors, assessments)`、`RiskFactorVO(label, value, detail)`；接口 `GET /api/trips/{tripId}/risk`。 |
...
| 涉及的文档和数据 | 实体 Trip、ItineraryNode、ExternalEvent、ImpactAssessment、AlternativePlan；表 `trips(id, status, start_date, end_date)`、`itinerary_nodes(trip_id, planned_start, planned_end, status)`、`impact_assessments(trip_id, assessment_hash, assessed_at)`、`alternative_plans(trip_id, source_assessment_hash, status)`；Redis `analysis:dedup:{tripId}:{assessmentHash}`、`lock:analysis:{tripId}`；接口对象 `ScheduledAnalysisJob`、`RiskSnapshotVO`。 |
...
| 涉及的文档和数据 | 实体 Trip、ItineraryNode、Route、ImpactAssessment、AlternativePlan、NodeChange；表 `alternative_plans(id, trip_id, title, strategy, extra_cost, extra_delay_minutes, changed_node_count, summary, status, source_assessment_hash)`、`node_changes(plan_id, original_node_id, change_type, new_place_name, new_start, new_end, new_cost, new_latitude, new_longitude, note)`；接口 DTO `ReplanResponse`、`AlternativePlanVO`、`NodeChangeVO`；接口 `POST /api/trips/{tripId}/replan`。 |
...
| 涉及的文档和数据 | 实体 AlternativePlan、PlanVote、GroupMember、NodeChange、ChangeLog、Trip；表 `alternative_plans(id, trip_id, status)`、`plan_votes(plan_id, member_id, choice, comment, voted_at)`、`node_changes(plan_id, ...)`、`change_logs(trip_id, description, extra_cost, refund_deadline, related_plan_id)`；接口 DTO `VoteRequest(memberId, choice, comment)`、`TallyResultVO(status, approveCount, rejectCount, abstainCount, trip)`；Redis `lock:vote:{planId}`、`idempotency:{key}`。 |
...
| 涉及的文档和数据 | 实体 ChangeLog、AlternativePlan、Trip；表 `change_logs(id, trip_id, description, extra_cost, refund_deadline, related_plan_id, created_at)`、`alternative_plans(id, title, strategy, status)`、`trips(id, version)`；接口 DTO `ChangeLogVO`、`PageResult<ChangeLogVO>`；接口 `GET /api/trips/{tripId}/changelogs`。 |
...
| 涉及的文档和数据 | 实体 Notification、User；表 `notifications(id, user_id, type, title, detail, payload, read_at, created_at)`、`users(id, status)`；接口 DTO `NotificationVO`、`NotificationPage`；接口 `GET /api/notifications`、`POST /api/notifications/{notificationId}/read`；STOMP 消息载荷含 `type/title/detail/payload`。 |
...
功能分组用例图：[社区管理用例图](uml/用例图_社区管理.png)。时序图：[社区管理时序图](uml/时序图_社区管理.png)；活动图：[社区管理活动图](uml/活动图_社区管理.png)。
...
| 涉及的文档和数据 | 实体 Comment、User；表 `comments(id, author_id, target_type, target_id, parent_id, content, mentions, like_count, deleted_at)`、`users(id, name, avatar)`；接口 DTO `CommentRequest(content, mentions)`、`CommentVO`；接口 `POST /api/trips/{tripId}/comments`、`POST /api/comments/{commentId}/like`。 |
| 后置条件 | 合法评论和互动状态已持久化。 |
...
| 涉及的文档和数据 | 实体 TravelGuide、GuideTemplate、GuideTemplateNode、GuideFavorite、GuideRating、User；表 `travel_guides(id, title, cover, author_id, city, tags, theme, days, per_capita_budget, season, rating, rating_count, favorite_count, view_count, status)`、`guide_templates(guide_id, title, total_days)`、`guide_template_nodes(template_id, day_offset, start_time, end_time, sequence_order)`、`guide_favorites(guide_id, user_id)`、`guide_ratings(guide_id, user_id, rating, comment)`；接口 DTO `GuideFilterRequest`、`TravelGuideVO`、`GuideDetailVO`。 |
...
| 涉及的文档和数据 | 实体 TravelGuide、GuideTemplate、GuideTemplateNode、Trip、ItineraryNode、Route；表 `travel_guides(id, status)`、`guide_templates(id, guide_id, total_days)`、`guide_template_nodes(template_id, name, place_name, latitude, longitude, node_type, day_offset, start_time, end_time, cost, sequence_order)`、`trips(group_id, start_date, source_guide_id)`、`itinerary_nodes(trip_id, planned_start, planned_end)`、`routes(trip_id, from_node_id, to_node_id)`；接口 DTO `ApplyGuideRequest(groupId, startDate, overrides)`、`ApplyGuideResponse(trip, constraintWarnings, trimmedNodeIds)`。 |
...
| 涉及的文档和数据 | 实体 Trip、ItineraryNode、ChangeLog、Comment、TravelGuide；表 `trips(id, group_id, status, start_date, end_date)`、`itinerary_nodes(trip_id, name, place_name, planned_start, planned_end, status)`、`change_logs(trip_id, description, extra_cost, related_plan_id)`、`comments(target_type, target_id, content)`、`travel_guides(title, author_id, description, status)`；接口 DTO `JournalDraftRequest`、`JournalPublishRequest`、`TravelGuideVO`。 |
...
功能分组用例图：[后台与监控管理用例图](uml/用例图_后台与监控管理.png)。时序图：[后台与监控管理时序图](uml/时序图_后台与监控管理.png)；活动图：[后台与监控管理活动图](uml/活动图_后台与监控管理.png)。
...
| 涉及的文档和数据 | 实体 User；表 `users(id, name, email, phone, role, status, deleted_at, created_at, updated_at)`；接口 DTO `AdminUserQuery(keyword, page, pageSize)`、`AdminUserStatusRequest(status)`、`PageResult<UserVO>`；接口 `GET /api/admin/users`。 |
...
| 涉及的文档和数据 | 实体 TravelGuide、Comment、ExternalEvent；表 `travel_guides(id, status, deleted_at, updated_at)`、`comments(id, target_type, target_id, content, deleted_at)`、`external_events(id, event_type, source, status, start_time, end_time, payload)`；接口 DTO `AdminReviewRequest(status, reason)`、`EventUpdateRequest(status, severity)`、`AnnouncementVO`。 |
...
| 涉及的文档和数据 | 实体 Trip、Bill、ExternalEvent、ImpactAssessment、AlternativePlan；表 `trips(status, start_date, end_date, total_budget, spent_budget)`、`bills(trip_id, amount, category, occurred_at)`、`external_events(event_type, severity, start_time, end_time)`、`impact_assessments(trip_id, event_id, risk_score, impact_level)`、`alternative_plans(trip_id, status, strategy, created_at)`；接口 DTO `AdminStatsQuery(from, to)`、`AdminStatsVO(tripCount, budgetDistribution, eventHitRate, replanAcceptanceRate, series)`。 |
...
| 成员 | 主要负责模块 | 交付内容 |
|---|---|---|
| 王一飞 | 用户管理、社区管理 | 注册登录与 JWT 鉴权、个人资料与设置、讨论区、攻略社区与评分收藏、攻略纳用、行后游记 |
| 祖建明 | 社交组队管理、出行规划管理 | 好友关系、出行小组与房间码、成员约束、初始方案生成、行程节点、地图路线、预算与分账 |
| 宋研 | 监测与应变管理、后台与监控管理、智能能力管理 | 外部事件接入、影响匹配、风险评分、自动重规划、投票与变更记录、通知与 WebSocket、持续动态分析、后台统计、全局旅行助手与 AI 辅助能力 |
