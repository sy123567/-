...
本文说明“行迹应变·多人出行动态规划与中断恢复平台”的总体设计、分层架构、模块设计、界面设计、数据库设计和综合技术考虑，为三人团队实现 Spring Boot 3 + MyBatis + MySQL 8 + Redis + React 的目标系统提供统一依据。
...
- 六大业务分组及其功能模块。
- 初始方案、影响匹配、风险评分、自动重规划、投票应用和攻略一键套用的核心流程。
...
“行迹应变·多人出行动态规划与中断恢复平台”是一个面向多人出行的动态规划和中断恢复系统。用户先通过好友和小组功能建立旅伴集合，再填写每位成员的日期、预算、体力、饮食、无障碍和必访地点约束。系统生成初始行程及路线，持续接收外部事件并分析其对未来节点的影响，计算风险并生成三类替代方案。成员通过实时投票达成共识后，系统原子应用变更、记录费用和退款截止时间，并向相关成员发送通知。

主要功能：

1. 账号管理：注册、登录、注销、忘记密码、个人资料管理。
2. 社交组队管理：好友关系管理、出行小组管理、房间码加入。
3. 出行规划管理：成员约束、初始方案、行程节点、地图路线、预算费用。
4. 监测与应变管理：事件接入、影响匹配、风险评分、持续分析、重规划、投票、变更、通知。
5. 社区管理：讨论区、攻略社区、一键套用、行后游记生成与发布。
6. 后台与监控管理：用户、运营、统计、可观测和配置。
...
| 后台与监控管理 | UserAdmin、Operations、Analytics、Observability |
...
| 社区管理 | `/guides`、`/guides/:id`、`/discussions` | 攻略筛选、模板路线、讨论和套用 | TravelGuide、TemplateNode、Comment | 搜索、收藏、一键套用、发布 |
