...
| [后端实现全流程](后端实现全流程.md) | 目标后端的分层、21 个模块、算法、Redis 和实时通信 |
...
领域类图给出跨模块的实体关系，各模块内部的 Controller、Service、Client、Repository 与实体的协作关系另见每个模块的独立类图：

| 模块 | 模块类图 |
|---|---|
| 账号管理 | [账号管理类图](uml/类图_账号管理.png) |
| 社交组队管理 | [社交组队管理类图](uml/类图_社交组队管理.png) |
| 出行规划管理 | [出行规划管理类图](uml/类图_出行规划管理.png) |
| 监测与应变管理 | [监测与应变管理类图](uml/类图_监测与应变管理.png) |
| 社区管理 | [社区管理类图](uml/类图_社区管理.png) |
| 智能能力管理 | [智能能力管理类图](uml/类图_智能能力管理.png) |
| 后台与监控管理 | [后台与监控管理类图](uml/类图_后台与监控管理.png) |
...
| 出行规划管理 | Constraint、Trip、Node、Route、Budget（含 AI 辅助行程生成，产物并入同一条节点与路线链路） |
| 监测与应变管理 | Event、Impact、Risk、Replan、Vote、Log、Notification |
| 社区管理 | Comment、Guide、GuideTemplate、Apply、Journal |
| 智能能力管理 | AssistantService、AiClient、AiConstraintService、检索上下文与降级策略 |
...
![监测与应变管理活动图](uml/活动图_监测与应变管理.png)
...
相关图： [账号管理用例图](uml/用例图_账号管理.png) · [账号管理时序图](uml/时序图_账号管理.png) · [账号管理活动图](uml/活动图_账号管理.png) · [账号管理类图](uml/类图_账号管理.png)
...
**相关类图：** 见[账号管理类图](uml/类图_账号管理.png)，实体关系另见[领域设计类图](uml/类图_领域模型.png)中的 User。
...
相关图： [社交组队管理用例图](uml/用例图_社交组队管理.png) · [社交组队管理时序图](uml/时序图_社交组队管理.png) · [社交组队管理活动图](uml/活动图_社交组队管理.png) · [社交组队管理类图](uml/类图_社交组队管理.png)
...
相关图： [出行规划管理用例图](uml/用例图_出行规划管理.png) · [出行规划管理时序图](uml/时序图_出行规划管理.png) · [出行规划管理活动图](uml/活动图_出行规划管理.png) · [出行规划管理类图](uml/类图_出行规划管理.png)
...
相关图： [监测与应变管理用例图](uml/用例图_监测与应变管理.png) · [监测与应变管理时序图](uml/时序图_监测与应变管理.png) · [监测与应变管理活动图](uml/活动图_监测与应变管理.png) · [投票决策补充活动图](uml/活动图_投票决策.png) · [监测与应变管理类图](uml/类图_监测与应变管理.png)
...
相关图： [社区管理用例图](uml/用例图_社区管理.png) · [社区管理时序图](uml/时序图_社区管理.png) · [社区管理活动图](uml/活动图_社区管理.png) · [社区管理类图](uml/类图_社区管理.png)
...
相关图： [后台与监控管理用例图](uml/用例图_后台与监控管理.png) · [后台与监控管理时序图](uml/时序图_后台与监控管理.png) · [后台与监控管理活动图](uml/活动图_后台与监控管理.png) · [后台与监控管理类图](uml/类图_后台与监控管理.png)
...
**简介：** 统一管理全局旅行助手、站内检索上下文、约束解析与替代地点智能提名。

相关图： [智能能力管理用例图](uml/用例图_智能能力管理.png) · [智能能力管理时序图](uml/时序图_智能能力管理.png) · [智能能力管理活动图](uml/活动图_智能能力管理.png) · [智能能力管理类图](uml/类图_智能能力管理.png)

**功能列表：**

- 全局旅行助手问答（`POST /api/ai/assistant`）。
- 攻略社区检索与功能目录导航作为回答上下文。
- AI 辅助行程地点提名（`GET /api/ai/plan`），产物并入出行规划的节点与路线链路。
- 成员约束文本化，供初始方案求解与替代地点筛选使用。

**代表功能设计描述：助手问答。**

AssistantService 先按标题、城市、主题、标签和正文对攻略做命中打分取前三篇，再匹配站内功能目录，把两份站内事实作为上下文交给 AiClient；模型不可达时直接用检索结果组织回答，响应体的 `source` 标记 `ai`/`local`/`offline`，API Key 只存服务端。

| 文件/组件 | 职责 |
|---|---|
| `AiController.java` | `/api/ai/**` |
| `AssistantService.java` | 攻略检索、目录匹配、提示词拼装 |
| `AiClient.java` | DeepSeek 调用与 JSON 解析 |
| `AiConstraintService.java` | 成员约束聚合与提示文本 |
| `ReplacementCandidateService.java` | 替代地点智能提名 |
| `AssistantDock.tsx` | 全局悬浮助手入口 |
